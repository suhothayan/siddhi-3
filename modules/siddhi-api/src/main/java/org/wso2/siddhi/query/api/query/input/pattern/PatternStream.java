...
public class PatternStream implements Stream, PatternElement {
    ...
    private HashSet<String> collectStreamIds(PatternElement patternElement,
        ...
        } else if (patternElement instanceof StandardStream) {
            streamIds.addAll(((WindowedStream) patternElement).getStreamIds());
        } else if (patternElement instanceof LogicalElement) {
            collectStreamIds(((LogicalElement) patternElement).getWindowedStream1(), streamIds);
            collectStreamIds(((LogicalElement) patternElement).getWindowedStream2(), streamIds);
        } else if (patternElement instanceof CountElement) {
            collectStreamIds(((CountElement) patternElement).getWindowedStream(), streamIds);
    ...
    public List<QueryEventSource> constructEventStreamList(PatternElement patternElement,
        ...
        if (patternElement instanceof StandardStream) {
            ((WindowedStream) patternElement).constructQueryEventSourceList(streamTableDefinitionMap, queryEventSources);
        } else if (patternElement instanceof LogicalElement) {
            constructEventStreamList(((LogicalElement) patternElement).getWindowedStream1(), streamTableDefinitionMap, queryEventSources);
            constructEventStreamList(((LogicalElement) patternElement).getWindowedStream2(), streamTableDefinitionMap, queryEventSources);
        } else if (patternElement instanceof CountElement) {
            constructEventStreamList(((CountElement) patternElement).getWindowedStream(), streamTableDefinitionMap, queryEventSources);
        } else if (patternElement instanceof FollowedByElement) {
